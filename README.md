...
* v0.3.6: **CSI TLS** — Default **`ocpSecretsStoreCsiVault.tls.projectedClusterCa.enabled`** is **`true`** again so the SecretProviderClass sets **`vaultCACertPath`** to the **`vp-sscsi-spc`** / **`openshift-sscsi-vault`** provider bundle path (default **`/etc/pki/vault-ca/ca-bundle.crt`**). Without it, the Vault CSI provider often uses only system roots and fails Vault login with **`x509: certificate signed by unknown authority`** even when **`rhvp.cluster_utils`** / proxy-cluster CA inject trust on the cluster. Deploy **`openshift-sscsi-vault`** before this app and use Argo sync waves if you see **`open ... ca-bundle.crt: no such file`**.

* v0.3.5: **CSI TLS** — **`vaultCaBundle`** no longer feeds **`vaultCACertPath`** in the SecretProviderClass. That parameter is read on the **Vault CSI provider** DaemonSet, not inside the workload pod; copying the app-namespace bundle path into the SPC caused **`open /etc/pki/vault-ca/ca-bundle.crt: no such file or directory`** when **`openshift-sscsi-vault`** (or equivalent) is not mounting that path on the provider. **`vaultCaBundle`** remains for CNO-injected trust in the job namespace and **`vaultTlsCheck`**. Omit **`tls.vaultCACertPath`** unless the provider has that file; use **`tls.projectedClusterCa.enabled: true`** when aligned with **`openshift-sscsi-vault`**, or **`tls.vaultSkipTLSVerify: "true"`** only where appropriate.

* v0.3.4: **CSI TLS (config-demo parity)** — Added **`vaultCaBundle`** ConfigMap with **`config.openshift.io/inject-trusted-cabundle`** (empty **`data`** + Argo **`ignoreDifferences`**) and projected mount on config job pods, matching **multicloud-gitops** `charts/all/config-demo`. **`vaultTlsCheck`** uses **`vaultCaBundle.mountPath`**/**`bundleKey`** when the bundle is enabled. (v0.3.4 briefly merged that path into the SPC; v0.3.5 corrects that — see v0.3.5 note.)
...
**CSI TLS on Vault login:** **`x509: certificate signed by unknown authority`** usually means the SecretProviderClass did not pass a cluster trust bundle to the Vault CSI **provider** (default was **`projectedClusterCa.enabled: false`** in v0.3.3–v0.3.5). With **`tls.projectedClusterCa.enabled: true`** (default again in **v0.3.6**), **`vaultCACertPath`** matches **`openshift-sscsi-vault`** on the provider (same trust chain **`rhvp.cluster_utils`** / proxy-cluster CA targets). **`open /etc/pki/vault-ca/ca-bundle.crt: no such file`** means the provider DaemonSet is not mounting that path yet — install or sync **`openshift-sscsi-vault`** first (Argo sync-wave lower than this application). **`vaultCaBundle`** is only for workload-namespace CNO trust and **`vaultTlsCheck`**; it does not configure the provider.
