...
**CSI mount error** `open /etc/pki/vault-ca/ca-bundle.crt: no such file or directory`: the Vault CSI **provider** opens **`vaultCACertPath`** from the SecretProviderClass on **its own filesystem**. The **`vaultCaBundle`** ConfigMap only mounts into **this chart's pods** (for **`vaultTlsCheck`**); it does **not** create that path on the provider. Fix: deploy **`openshift-sscsi-vault`** (or match its provider mount) and then set **`tls.vaultCACertPath`** or **`tls.projectedClusterCa`** per **`vp-sscsi-spc`** / that chart; or omit **`vaultCACertPath`** (leave **`tls.vaultCACertPath`** empty and **`projectedClusterCa.enabled`** false) so the SPC does not reference a missing file; or (lab only) **`tls.vaultSkipTLSVerify: "true"`**.
...
| ocpSecretsStoreCsiVault.tls.projectedClusterCa.enabled | bool | `false` |  |
